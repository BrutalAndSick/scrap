   ...
   if ($r && count($existe)>0 && $activo=='') 
       { //LOGIN CORRECTO
			...
			for($i=0; $i<$entries["count"]; $i++) {
				...
				$_SESSION['TYPE']	  = $tipo_; } 
				switch($tipo_) {
					case "administrador"	:	header("Location: admin/inicio.php"); 
												break;
					case "capturista"		:	header("Location: capturista/inicio.php"); 
												break;
					case "autorizador"		:	header("Location: autorizador/scrap_firmar.php"); 
												break;	
					case "reportes"   		:	header("Location: reportes/rep_general.php"); 
												break;																								
				}
		}	
...
if(get_logueado($user_)=='SI')  {
   echo "<META HTTP-EQUIV=\"Refresh\" CONTENT=\"0; URL=index.php?error=6&user_=$user_&tipo_=$tipo_\">"; 
	...
	switch($tipo_) {
		...
		case "administrador"	:	
			$s_1 = "select * from empleados where ((usuario='$user_' and password='$pass_' and activo='1' and administrador!='0') ";
    ...
    <td valign="top">
		...
		<tr height="30">
			...
			<select name="tipo_" class="texto" style="width:131px;">
				<option value="administrador" <?php if($tipo_=='administrador'){?> selected="selected"<?php } ?>>
                Administrador</option>
				<option value="autorizador" <?php if($tipo_=='autorizador'){?> selected="selected"<?php } ?>>
                Autorizador</option>
				<option value="capturista" <?php if($tipo_=='capturista'){?> selected="selected"<?php } ?>>
                Capturista</option>	
				<option value="reportes" <?php if($tipo_=='reportes'){?> selected="selected"<?php } ?>>
                Reportes</option>
			</select>	
			</td>
