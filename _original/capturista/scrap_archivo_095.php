...
function exportar() {
	form2.action = 'excel_ayudas.php?op=codigo_095';
	form2.submit();
	form2.target = '_self';
	form2.action = 'scrap_archivo_095.php?op=guardar';
}
</script>
</head>
...
<table width="90%" border="0" align="center" cellpadding="0" cellspacing="0">
  ...
  <tr>
    <td><?php submenu('b_capturae','b_captura095'); ?></td>
    ...
    <td>&nbsp;
		...
		<?php switch($op) {
			...
			case "cancelar"	:	$s_ = "delete from scrap_folios_tmp where id_emp='$_SESSION[IDEMP]'";
								...
								$r_ = mysql_query($s_); 
								$s_ = "delete from scrap_partes_095 where id_emp='$_SESSION[IDEMP]'";
								$r_ = mysql_query($s_);
...
<tr>
	<td class="titulo" align="left">CAPTURA POR ARCHIVO PARA CÓDIGO 095</td>
	<td align="left">&nbsp;&nbsp;&nbsp;&nbsp;
	...
	while ( !feof($fd) ) 
 	{
		...
		$s_1 = "Select proyectos.id, nombre from proyectos, codigo_scrap_proy where nombre like '".trim($campos[7])."' and id_planta='$folios[8]' ";
		$s_1.= "and id_division='$folios[10]' and id_segmento='$folios[12]' and id_pc='$folios[14]' and activo='1' and codigo_scrap_proy.codigo='095' and ";
		...
		$d_1 = mysql_fetch_array($r_1);				
			...
			$folios[28] = "095"; //Código Scrap
			$folios[29] = "0"; //Financiero
			$folios[30] = "1095"; //Reason Code
			//Verificar que Orden Interna le corresponde
			/*$s_1 = "Select orden_interna from oi_especial where id_division like '".trim($folios[10])."' and id_segmento='".trim($folios[12])."'  ";
			$s_1.= "and id_pc='".trim($folios[14])."' and id_proyecto='".trim($folios[6])."' and activo='1'"; 
			$r_1 = mysql_query($s_1);
			if(mysql_num_rows($r_1)>0) {
				$d_1 = mysql_fetch_array($r_1);		*/
			$folios[31] = "K96019"; //Orden Interna
			/*} else { $insertar++; $error[$e].= "La división, segmento, profit center y el proyecto no están ligados a la orden interna especial: ";
					 $error[$e].="($campos[4] $campos[5] $campos[6] $campos[7]).<br>"; 
			}*/
			$folios[32] = "MB1A"; //TXS SAP
		...
		if(mysql_num_rows($r_1)<=0) {
			...
			$query  = "INSERT into scrap_partes_095 values('', '$partes[papa]', '$partes[num_p]', '$partes[tipo]', '$partes[subt]', ";
...
<?php $s_ = "select * from scrap_partes_095 where id_emp='$_SESSION[IDEMP]' order by id asc";
...
$s_3 = "select * from scrap_partes_095 where proyecto='$d_[id_proyecto]' and linea='$d_[id_linea]' and apd='$d_[id_apd]' and id_emp='$_SESSION[IDEMP]' and ";
...
if(mysql_num_rows($r_3)>0) { 
	...
	//Aplica misma regla de usuarios de merma entoncés reviso si es un usuario que sólo requiere la autorización de inventarios
	if(capt_merma($_SESSION['IDEMP'])=="SI"){ 
		...
		//Si el código de scrap es 095 y el perfil es inventarios, se autoriza automático.
