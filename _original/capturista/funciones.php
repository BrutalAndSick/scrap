...
/**/function aplica_lo_loa($planta,$codigo_scrap,$division,$prce,$area,$proyecto) {
	//Obtener los departamentos que deben autorizar en base al c�digo de scrap
	//No aplica el archivo en el caso de c�digo de scrap: 064-2, 025-4 y 062-4
...
/**/function validar_autorizadores($planta,$codigo_scrap,$division,$prce,$area,$proyecto) {
	$mensaj_1 = 'Hay un error con los autorizadores asignados al c�digo de scrap seleccionado. Contacte al administrador del sistema.';
	...
	//Si el proyecto no requiere autorizaci�n especial:
	$s_1 = "select apr_especial from proyectos where id='$proyecto'";
	$r_1 = mysql_query($s_1);
	$d_1 = mysql_fetch_array($r_1); $apr_especial = $d_1['apr_especial'];
	
	if($apr_especial!='si') { 
	//1.Obtener los departamentos que deben autorizar en base al c�digo de scrap que no sean LPL
	...
	if(mysql_num_rows($r_1)>0) { 
		while($d_1=mysql_fetch_array($r_1)) {
		//Revisar si existen personas asignadas a cada nivel de autorizaci�n
		...
		if($d_1['tipo']=='prod') { 
			//Debe existir un responsable para producci�n por �rea y divisi�n
			$s_ = "select autorizadores.* from autorizadores, empleados where tipo='$d_1[tipo]' and id_division='$division' and ";
	...
	echo"<script>alert('$alerta_1');</script>";	
		...
		//Revisar si existen personas asignadas a cada nivel de autorizaci�n (menor a 50000 y mayor a 5000) y a este proyecto (o todos) a�n no sabemos el monto total
...
/**/function autorizaciones($folio,$planta,$codigo_scrap,$proyecto,$total) {
	...
	//Revisar si el folio requiere de autorizaci�n OES (si alg�n global pc del material termina con 056)
	...
	if($apr_especial!='si') {  
		...
		/*Si no se ha insertado SQM y el c�digo de scrap es 10-3, 10-4, 10-5, 10-6, 12-1 � 13-1 se requiere SQM obligatorio*/
		...
		/*Si el monto es mayor o igual a $100,000 requiere autorizaci�n de FFM*/
		if($total>=100000) {
