...
function guardar_1($partes,$partes_name,$comentario,$accion_correctiva) {
	...
	echo "-->Copiando archivo a temporal: $nom_final_<br>";
	...
	echo "-->Entrando a la función de insert_csv<br>";
...
function insert_csv($nombre_,$comentario,$accion_correctiva) {
	...
	echo "-->Leyendo archivo ".$r_server."partes_UID".$_SESSION["IDEMP"].".csv<br>";
	...
	while ( !feof($fd) ) 
 	{
		...
		$s_1 = "Select id, nombre from proyectos where nombre like '".trim($campos[7])."' and id_planta='$folios[8]' and id_division='$folios[10]' and ";
		...
		} else { $insertar++; $error[$e].= "El nombre de la línea no existe para el área y estación: $campos[8] ($folios[19], $folios[21]).<br>"; }
		...
		if(mysql_num_rows($r_1)<=0) {
			...
			echo "-->Insertanto query ".$query."<br>";
		/*} else { 
	...
	echo "-->Entrado a listado temporal<br>";
	listado_temporal();
}
