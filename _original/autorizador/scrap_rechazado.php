...
function validar(destino) {
	if(destino=='excel') { 
		form1.action='excel_reportes.php?op=consulta&tipo=rechazado';
		form1.submit();	
  ...
  <tr>
    <td><?php submenu('b_consulta','rechazados'); ?></td>
...
<tr>
	<td class="titulo" width="190">SCRAP RECHAZADO</td>
	<td align="left">&nbsp;
...
<table align="center" class="tabla">
	<tr height="20">
        ...
        <td><?php $s_1 = "select campo, nombre from encabezados where a_rechazados='1' order by nombre"; 
				  $r_1 = mysql_query($s_1); ?>
...
<tr onMouseOut="this.style.background='#F7F7F7'" onMouseOver="this.style.background='#FFDD99'" bgcolor="#F7F7F7" height="20">
    ...
    <td align="center">
 	<?php $s_b = "select status from aut_bitacora where no_folio='$d_1[no_folio]' order by id desc";
	      $r_b = mysql_query($s_b);
		  $d_b = mysql_fetch_array($r_b); 
		if($d_b['status']=='5') { ?>
        	<img src="../imagenes/tick.png" border="0"><?php } else { echo "&nbsp;"; } ?></td> 
