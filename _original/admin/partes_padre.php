...
function exportar() {
	form1.action='excel.php?op=partes_padre';
	form1.submit();
...
<tr height="600" valign="top">
	...
	<?php menu_interno();
		...
		case "listado"			:	listado($type,$apd,$buscar,$pagina,$tipo,$f_nivel); break;
		case "estado"			:	estado($id_1,$id_2,$id_3,$estado,$type,$apd,$buscar,$pagina); 
									listado($type,$apd,$buscar,$pagina,$tipo,$f_nivel); break;
		case "editar"			:	editar($id_1,$id_2,$id_3,$type,$tipo,$apd,$buscar,$pagina); break;
		...
		case "borrar"			:	borrar($id_1,$id_2,$id_3,$type,$apd,$buscar,$pagina); 
									listado($type,$apd,$buscar,$pagina,$tipo,$f_nivel); break;		
		default					:	listado($type,$apd,$buscar,$pagina,$tipo,$f_nivel); break;
	} ?>	
	<!-- -->
	<br><br><br></td>
...
function listado($type,$apd,$buscar,$pagina,$tipo,$f_nivel) {
...
$r_ = mysql_query($s_);
   ...
   if($apd!='' || ($buscar!='' && strlen($buscar)>=5)) { 
	$s_1 = "CREATE OR REPLACE VIEW vw_padre AS SELECT * from partes_padre where activo!='2' and padre!='' and material!='' "; 
    $r_1 = mysql_query($s_1); 	
  	 if($tipo=='padre' && $buscar!='')    { $s_1.= "and padre like '$buscar%' "; }
  	 if($tipo=='material' && $buscar!='') { $s_1.= "and material like '$buscar%' "; }
	 if($f_nivel!='') { $s_1.= "and nivel='$f_nivel' "; }
...
<div align="center" class="aviso">Los encabezados de la tabla permiten ordenar los campos. Debe especificar la búsqueda por APD y/o material, parte padre.</div>
<form action="?op=listado" method="post" name="form2">
<input type="hidden" name="type" value="<?php echo $type;?>">
<input type="hidden" name="orden" value="<?php echo $orden;?>">
<table align="center" class="tabla">
<tr>
	<td width="50" align="center">Buscar:</td>
	<td><input type="text" class="texto" name="buscar" value="<?php echo $buscar;?>" size="20"></td>
    <td width="30" align="center">En:</td>
    <td width="110" align="center"><select name="tipo" class="texto" style="width:100px;">
    	<option value=""></option>
        <option value="padre" <?php if($tipo=='padre'){?> selected="selected"<?php } ?>>Parte Padre</option>
        <option value="material" <?php if($tipo=='material'){?> selected="selected"<?php } ?>>Material</option>
        </select></td>    
    ...
    <td><select name="f_nivel" class="texto" style="width:80px;">
    	<option value="">Todos</option>
        <option value="0" <?php if($f_nivel=='0'){?> selected="selected"<?php } ?>>0</option>
        <option value="1" <?php if($f_nivel=='1'){?> selected="selected"<?php } ?>>1</option>
        <option value="2" <?php if($f_nivel=='2'){?> selected="selected"<?php } ?>>2</option>
        <option value="3" <?php if($f_nivel=='3'){?> selected="selected"<?php } ?>>3</option>
        </select></td>    
    ...
    <td width="120" align="center"><input type="button" class="submit" value="Exportar" onclick="exportar();" <?php if($tot<=0) { ?> disabled <?php } ?>></td>
</tr>
</table><br>
</form>
...
<?php if($apd!='' || ($buscar!='' && strlen($buscar)>=5)) {
   ...
   $ruta.= "&tipo=$tipo&apd=$apd&buscar=$buscar"; ?>
<tr onMouseOut="this.style.background='#F7F7F7'" onMouseOver="this.style.background='#FFDD99'" bgcolor="#F7F7F7" height="20">
	<td align="center"><?php echo $j;?></td>
	<td align="center">
		<?php if($_SESSION["TYPE"]=='administrador') { 
		   if($d_1['activo']=='1') { echo"<a href='?op=estado$ruta&estado=0'><img src='../imagenes/tick.png' alt='Activo' border='0'>";    } 
		   if($d_1['activo']=='0') { echo"<a href='?op=estado$ruta&estado=1'><img src='../imagenes/cross.png' alt='Inactivo' border='0'>"; } } else { 
	...
	<td align="center">
    	<?php if($_SESSION["TYPE"]=="administrador") { ?>
    	<a href="?op=editar<?php echo $ruta;?>"><img src="../imagenes/pencil.gif" alt="Editar" border="0"></a><?php } else { ?>
        <img src="../imagenes/pencil_gris.gif" alt="No puede editar" border="0"><?php } ?></td>
	<td align="center">
    	<?php if($_SESSION["TYPE"]=="administrador") { ?>
        <a href="?op=borrar<?php echo $ruta;?>" onclick='return confirm("¿Borrar Registro?")'><img src="../imagenes/delete.gif" alt="Borrar" border="0"></a>
        <?php } else { ?><img src="../imagenes/delete_gris.gif" alt="No puede editar" border="0"><?php } ?></td>
</tr>
<?php $j++; } } ?>
</tbody>
</table>
...
function editar($id_1,$id_2,$id_3,$type,$tipo,$apd,$buscar,$pagina) {
...
<tr>
	<td valign="top" align="left">Tipo:*</td>
	<td valign="top">	
		<select name="type" style="width:180px;" class="texto">
			<option value=""></option>
...
function update($id_1,$id_2,$id_3,$type,$apd,$buscar,$pagina,$padre,$apd_,$tipo,$nivel,$material) {
	...
	$material = trim($material);
	 ...
	 $s_1 = "update partes_padre set padre='$padre', apd='$apd_', material='$material', type='$type', nivel='$nivel' where ";
