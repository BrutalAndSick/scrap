...
function exportar() {
	form2.action='excel.php?op=capturistas';
	form2.submit();	
	form2.action='capturistas.php?op=listado';
}
</script>
  ...
  <tr>
    <td><?php submenu('b_usuarios','capturistas'); ?></td>
...
<tr>
	<td class="titulo" width="200">CAPTURISTAS</td>
</tr>
</table></div><hr>
<?php } 	
...
<?php }
	$s_ = "select empleados.usuario, empleados.nombre, empleados.apellidos, capturistas.* from capturistas, empleados ";
	$s_.= "where empleados.id = capturistas.id_emp and capturistas.id_division like '$f_division%' and empleados.capturista='1' ";
...
function agregar($f_division,$division,$usuario) {
	$s_ = "select * from capturistas where id_emp='$usuario' and id_division='$division'"; 
	$r_ = mysql_query($s_);
	if(mysql_num_rows($r_)<=0) { 
		$s_1 = "Insert into capturistas values('', '$usuario', '$division')";
		$r_1 = mysql_query($s_1); }
	else { 
...
function eliminar($f_division,$id_) {
	$s_ = "delete from capturistas where id='$id_'";
	$r_ = mysql_query($s_);
}?>