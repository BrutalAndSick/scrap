...
switch ($strProcess) {
    case 'getWSUserData':
        ...
        $jsnPhpScriptResponse = $objScrap->getWSUserData($strPersonalNumber);
        break;
    case 'updateGrid':
        ...
        $objScrap->intScrollPosition = $_REQUEST['intScrollPosition'];
        $objScrap->updateGrid();
        $jsnPhpScriptResponse['grid'] = $objScrap->strGrid;
        $jsnPhpScriptResponse['pagination'] = $objScrap->strGridPagination;
        $jsnPhpScriptResponse['intSqlNumberOfRecords'] = $objScrap->intGridNumberOfRecords;
        $jsnPhpScriptResponse['intScrollPosition'] = $objScrap->intScrollPosition;
        break;
    case 'processRecord':
            ...
            if ($jsnForm[$intIndex]['TBL_DUPLICATE'] == 0) {
                ...
                if ($objScrap->getProperty('strDBError') == '') {
                    if ($rstRecordCount[0]['COUNT'] != 0) {
                        ...
                        $jsnPhpScriptResponse['strError'] = $jsnForm[$intIndex]['TBL_NAME'] . " <b>" . $_REQUEST[$jsnForm[$intIndex]['TBL_FIELD']] . "</b> ya existe";
                        $jsnPhpScriptResponse['strField'] = $jsnForm[$intIndex]['TBL_FIELD'];
                        $intIndex = count($jsnForm);
                    }
                } else {
                    $jsnPhpScriptResponse['blnGo'] = false;
                    $jsnPhpScriptResponse['strError'] = $objScrap->getProperty('strDBError');
                    $intIndex = count($jsnForm);
                }
    ...
    case 'generateImportTemplate':
        ...
        if(count($arrRelation)>0){
            foreach($arrRelation as $intRelationIndex=>$objRelation){
                ...
                $objPHPExcel->setActiveSheetIndex($intRelationIndex + 1)->setCellValueByColumnAndRow(0,1, $objRelation['TBL_DISPLAY']);
                $objPHPExcel->getActiveSheet()->getStyle('A1:Z1')->applyFromArray($arrStyle);
                ...
                if(strlen($objRelation['TBL_DISPLAY'])>31){
                    $objPHPExcel->getActiveSheet()->setTitle(substr($objRelation['TBL_DISPLAY'],0,29) . '...');
                }else{
                        ...
                        if(!$blnCellValueError){
                            ...
                            foreach($objRow as $intColIndex=>$objCell) {
                                ...
                                if($intColIndex<count($arrFormField)){
                                    ...
                                    $strSqlInsertRecord = "INSERT INTO " . $objScrap->strTableName . "(" . substr($strInsertRecordFields,0,strlen($strInsertRecordFields) - 1) . ") VALUES(" . substr($strInsertRecordValues,0,strlen($strInsertRecordValues) - 1) . ")";
                                    $objScrap->dbInsert($strSqlInsertRecord);
                                    if(count($arrFormField)>1){
                                        $strSqlUpdateRecord = "UPDATE " . $objScrap->strTableName . " SET " . substr($strUpdateValues,0,strlen($strUpdateValues) - 1) . " WHERE " . $strUpdateNeedle;
                                        $objScrap->dbUpdate($strSqlUpdateRecord);
                                    ...
                                    if($intColIndex<$intNumberOfColumns){
                                        ...
                                        unset($rstExcelRelation);
                                        if($intRelationID!=0){
                                            $strSqlRecordId = "SELECT " . $objScrap->strTableIdField . " AS FIELDID FROM " . $objScrap->strTableName . " WHERE " . $strUpdateNeedle;
                                            echo $strSqlRecordId . "<br />";
                                            $rstRecordId = $objScrap->dbQuery($strSqlRecordId);
                                            $intRecordId = $rstRecordId[0]['FIELDID'];
                                            unset($rstRecordId);
                                            $strSqlInsertRelation = relationTable($arrTableRelation[$intColIndex - count($arrFormField)]['TBL_NAME'],"INSERT INTO ||strTable||(||strField_0||,||strField_1||) VALUES(" . $intRecordId . "," . $intRelationID . ")");
                                            echo $strSqlInsertRelation . "<br /><br />";

                                            $objScrap->dbInsert($strSqlInsertRelation);
                                        }
                                    }else{
                                        break;
                                    }
