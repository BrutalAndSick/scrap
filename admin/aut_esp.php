...
function exportar() {
	form2.action='excel.php?op=aut_esp';
	form2.submit();	
	form2.action='aut_esp.php?op=listado';
}
</script>
...
<tr height="600" valign="top">
	...
	<?php menu_interno();
	switch($op) {
		case "listado"	:	listado($f_tipo,$f_proyecto,$pagina); break;
		case "guardar"	:	guardar($f_tipo,$f_proyecto,$tipo,$proyecto,$usuario); listado($f_tipo,$f_proyecto,$pagina); break;
		case "borrar"	:	borrar($f_tipo,$f_proyecto,$id_); listado($f_tipo,$f_proyecto,$pagina); break;
		default			:	listado($f_tipo,$f_proyecto,$pagina); break;
	} ?>	
	<!-- -->
	<br><br><br></td>
...
<tr>
	...
	<td><img src="../imagenes/division_submenu.png"></td>
	<td><a href="aut_esp.php" class="naranja2">Aprobación Especial</a></td>
	<td><img src="../imagenes/division_submenu.png"></td>
...
function listado($f_tipo,$f_proyecto,$pagina) {
...
<tr>	
	<td>Tipo:</td>
	<td><select name="tipo" style="width:350px;" class="texto">
      <option value=""></option>
	  <option value="esp_1">Menor a 50,000</option>
	  <option value="esp_2">Mayor a 50,000</option>
	  </select></td>
</tr>
<tr>	
	<td>Proyecto:</td>
	<td><select name="proyecto" style="width:350px;" class="texto">
	  ...
	  <?php $s_ = "select divisiones.nombre as division, proyectos.* from proyectos, divisiones where divisiones.activo='1' and proyectos.activo='1' and divisiones.id = ";
	        $s_.= "proyectos.id_division and proyectos.apr_especial='si' order by proyectos.nombre";
	     $r_ = mysql_query($s_);
...
<tr>		
	...
	<td><select name="usuario" style="width:350px;" class="texto">
      <option value=""></option>
	  <?php $s_ = "select * from empleados where autorizador='esp' and activo='1' order by apellidos, usuario";
	     $r_ = mysql_query($s_);
		 while($d_=mysql_fetch_array($r_)) { ?>
...
<?php }
	...
	$s_.= "autorizadores.id_emp = empleados.id and (autorizadores.tipo = 'esp_1' or autorizadores.tipo = 'esp_2') and tipo like '$f_tipo' ";
	if($f_proyecto!='') { $s_.= "and id_proyecto='$f_proyecto' "; }
...
<tr>
	...
	<tr bgcolor="#E6E6E6" height="20">
        ...
        <select name="f_tipo" style="width:250px;" class="texto" onchange="submit();">
          <option value="">Tipo de aprobación</option>
          <option value="esp_1" <?php if($f_tipo=="esp_1"){?> selected="selected"<?php } ?>>Menor a 50,000</option>
          <option value="esp_2" <?php if($f_tipo=="esp_2"){?> selected="selected"<?php } ?>>Mayor a 50,000</option>
          </select>	
		</td>
        <td width="180" align="center">
        <select name="f_proyecto" style="width:250px;" class="texto" onchange="submit();">
          <option value="">Proyecto</option>
           <option value="%" <?php if($f_proyecto=='%'){?> selected="selected" <?php } ?>>TODOS</option>
		  <?php $s_1 = "select divisiones.nombre as division, proyectos.* from proyectos, divisiones where divisiones.activo='1' and proyectos.activo='1' and ";
                $s_1.= "divisiones.id = proyectos.id_division and proyectos.apr_especial='si' order by proyectos.nombre";
             $r_1 = mysql_query($s_1);
	...
	<tr onMouseOut="this.style.background='#F7F7F7'" onMouseOver="this.style.background='#FFDD99'" bgcolor="#F7F7F7" height="20">
        ...
        <td align="left">&nbsp;&nbsp;<?php if($d_['tipo']=="esp_1"){ echo "Menor a 50,000"; } else { echo "Mayor a 50,000"; } ?></td>
...
function guardar($f_tipo,$f_proyecto,$tipo,$proyecto,$usuario) {
if($division!='%') { 
	$s_ = "select * from autorizadores where id_emp='$usuario' and tipo='$tipo' and id_proyecto='$proyecto'"; 
	$r_ = mysql_query($s_);
	if(mysql_num_rows($r_)<=0) { 
		$s_1 = "Insert into autorizadores values('', '0', '0', '0', '$proyecto', '$tipo', '$usuario')";
		$r_1 = mysql_query($s_1); }
	else { 
		echo "<script>alert('El usuario seleccionado ya está asignado al proyecto y tipo');</script>"; } }
else { 
	$s_ = "delete from autorizadores where id_emp='$usuario'";
	$r_ = mysql_query($s_);		
	$s_ = "Insert into autorizadores values('', '0', '0', '0', '$proyecto', '$tipo', '$usuario')";
	$r_ = mysql_query($s_); }
}


function borrar($f_tipo,$f_proyecto,$id_) {
